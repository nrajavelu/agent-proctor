...
export default function QuizInterface({ candidateData, onLogout }: QuizInterfaceProps) {
  ...
  useEffect(() => {
    // Load ProctorSDK script dynamically
    const script = document.createElement('script');
    script.src = '/packages/proctoring-sdk/dist/proctor-sdk.umd.js';
    script.onload = () => {
  ...
  useEffect(() => {
    if (proctoringEnabled && window.ProctorSDK) {
      try {
        const proctor = window.ProctorSDK.init({
          ...
          widgetUrl: '/packages/proctoring-widget/dist/widget.html',
          position: 'top-right',
          autoStart: true
        });
